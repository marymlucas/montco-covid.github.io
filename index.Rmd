...
No noticeable spike here. The ongoing drop in calls is concerning because it may imply people are now reluctant to call 911 and go to hospital for fear of getting exposed to COVID-19.
...
### 911 Calls for Cardiac Arrests
Fairly consistent

```{r, echo=FALSE, warning=FALSE, message=FALSE}
# Calls for Cardiac Arrests
tz_latest_arrest <- tz_latest %>% filter(title == "EMS: CARDIAC ARREST") %>%
  group_by(month=floor_date(datetime, "week")) %>%
  tally(name = 'number_of_calls_arrest') 

ggplot(tz_latest_arrest, aes(x=as.Date(month), y=number_of_calls_arrest)) +
  geom_bar(stat = "identity") + scale_x_date(date_labels = "%m-%Y", date_breaks = "2 months", limits = lims) + geom_smooth(method = "gam", color = "red", se = FALSE) +
  theme_minimal() +
  #labs(title = "911 Calls for Cardiac Arrests", subtitle = "Montgomery County, PA") +
  xlab("Month -Year") +
  ylab("Number of Calls") +
  theme(axis.text.x=element_text(angle=60, hjust=1))
```
